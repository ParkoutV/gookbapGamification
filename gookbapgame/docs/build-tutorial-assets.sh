...
#
# **주의: 지금 리포에 있는 세 장은 이 스크립트가 만든 것이 아니다**(2026-08-19).
#   이란토가 what.webp 기준으로 해상도를 630×953으로 통일해 직접 다시 구성했다.
#   그대로 돌리면 옛 크롭으로 덮어쓴다 — 되살릴 일이 있으면 크롭 값부터 새 구성에
#   맞춰 고칠 것. 화면 쪽은 이제 비율을 알지 않으므로(폭만 주고 높이는 브라우저가
#   애셋에서 읽는다) 애셋 크기가 바뀌어도 코드는 고칠 것이 없다.
#
...
#   것은 의도된 것이었고, 예전에는 화면이 그 비율을 `TUTORIAL_SHOTS.aspect`로 들고
#   있었다 — 애셋을 다시 만들 때마다 그 숫자를 같이 고쳐야 해서 2026-08-19에 없앴다.
#
# 왜 마커(링·화살표)를 굽지 않나:
