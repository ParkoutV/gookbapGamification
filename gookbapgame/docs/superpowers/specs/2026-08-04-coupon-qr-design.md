...
- **`surveyIntro`**: "설문에 답하면 쿠폰 룰렛을 돌릴 수 있어요"를 명시하고 참여/거절 2버튼을 제공한다. `survey_questions`에 `survey_phase = 1` 문항이 **0개면 이 화면과 `survey`를 모두 건너뛰고 곧장 `wheel`로 간다**(문항을 등록하지 않은 운영 상태에서도 쿠폰 발급은 동작해야 하므로).
