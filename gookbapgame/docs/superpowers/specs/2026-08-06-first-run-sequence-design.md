...
위해서다. X 자체는 `preloadStatus`를 건드리지 않지만, 다시 시작하면 `runPreload`가 첫
문장에서 `loading`으로 리셋하고 재요청하므로 **프리로드는 처음부터 다시 돈다**
(`preloadAllStages`에 캐시가 없다).

X 이탈은 프리로드 경합을 만들 수 있는 유일한 경로이기도 하다 — 나갔다가 곧바로 다시
시작하면 두 요청이 동시에 진행되고, 늦게 끝난 쪽이 이미 플레이 중인 `sessions`와
`loadNonce`를 덮어써 그림이 갈린다. `runPreload`의 generation 카운터가 이걸 막는다:
매 호출이 세대 번호를 증가시키고, `await` 이후 자신이 최신 세대일 때만 상태를 갱신한다.
...
(`track_logs`는 2026-08-09부터 최초 1회가 아니라 **접속할 때마다** 기록된다 — 아래 판단은
기록 시점에 대한 것이므로 빈도가 바뀌어도 그대로 유효하다.)
