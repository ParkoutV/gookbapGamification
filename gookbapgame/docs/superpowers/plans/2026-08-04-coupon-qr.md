...
export default function Home({ searchParams }: PageProps) {
  ...
  // 룰렛 재시도용. onRetry로 값을 올려 WheelScreen을 강제 리마운트한다 —
  // phase가 계속 "wheel"이라 리마운트 없이는 마운트 시 실행되는 onSpin이 다시 돌지 않는다.
  ...
  const handleWheelRetry = useCallback(() => {
    resetCoupon();
    setRetryNonce((n) => n + 1);
  }, [resetCoupon]);
    ...
    <div className="min-h-screen bg-black">
      ...
      {game.phase === "surveyIntro" && (
        <SurveyIntroScreen
          ...
          onDecline={proceedToDailyResult}
        />
      )}
      ...
      {game.phase === "wheel" && (
        <WheelScreen
          ...
          onSpin={spin}
          onRetry={handleWheelRetry}
          onNext={proceedToDailyResult}
        />
      )}
...
`key={retryNonce}`가 있어야 하는 이유: `onRetry`는 phase를 바꾸지 않으므로(계속 `"wheel"`) `WheelScreen`이 언마운트되지 않고, 마운트 시 1회 실행되는 `onSpin`도 다시 돌지 않는다. `key`를 올려 강제 리마운트하는 것이 이 흐름에서 재시도를 성립시키는 유일한 장치다 — 지우지 말 것.

Run: `npx tsc --noEmit && npm run build`
Expected: 빌드 통과.

그 다음 로컬에서 `GATCHA_DRAW_API_URL`을 잘못된 주소로 두고 룰렛에 진입해 에러 화면을 띄운 뒤 "다시 시도"를 누른다.
Expected: 다시 `wheel.spinning`이 뜬다.
