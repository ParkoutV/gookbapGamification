...
- [ ] **Step 5: draw 실패 후 재진입 경로가 동작하는지 확인한다**

전용 재시도 버튼이 없으므로, 요청이 서버에 닿지 못했을 때의 복구는 재진입 경로가 전담한다. `handleSurveyAgain`이 `resetCoupon()`으로 `drawStartedRef`를 풀고, phase가 `surveyIntro`를 거쳐 다시 `wheel`이 될 때 `WheelScreen`이 새로 마운트되면서 `onSpin`이 다시 실행된다.

Run: `npx tsc --noEmit && npm run build`
Expected: 빌드 통과.

그 다음 로컬에서 `GATCHA_DRAW_API_URL`을 잘못된 주소로 두고 룰렛에 진입해 `wheel.error`를 띄운다. "다음"으로 오늘의 결과까지 간 뒤, 환경변수를 올바르게 고치고 서버를 재시작한 다음 "설문하고 쿠폰 받기"를 누른다.
Expected: 설문 안내 → 문항 화면이 다시 뜨고, 제출을 누르면 `hasSubmittedRef` 때문에 서버 호출 없이 즉시 룰렛으로 넘어가 `wheel.spinning` 후 쿠폰이 발급된다(응답이 두 번 쌓이지 않는다).
