...
**행 구조는 대시보드가 실제로 읽는 형태를 그대로 따른다.** `gookbapanalyze/app/main/survey-results/actions.ts:54`가 조회하는 컬럼은 `response_id, question_id, participant_id, answer_data, created_at`이며, 답변은 `answer_data`(jsonb) **한 컬럼**에 들어간다. 문항 하나당 행 하나다.

`SurveyResultsClient.tsx`의 렌더러가 해석하는 형태:

| type | `answer_data` | 근거 |
|---|---|---|
| 0 | 선택한 옵션 인덱스 (number) | `renderType0`: `String(r.answer_data)`를 옵션 인덱스 키와 대조 |
| 1 | 선택한 인덱스 배열 (number[]) | `renderType1`: `Array.isArray(r.answer_data)`면 `map(String)` |
| 2 | 입력 문자열 (string) | `renderType2`: `typeof === 'string'`이면 그대로 사용 |
...
- Produces:
  ...
  - `export type SurveyResponseRow = { question_id: number; answer_data: number | number[] | string }`
...
- [ ] **Step 4: RPC의 반환 컬럼과 정렬 순서를 확인한다**

`drawCoupon()`이 `coupons[0]`을 "방금 발급된 쿠폰"으로 쓴다. 순서가 반대면 **오래된 쿠폰의 QR을 새 당첨 상품으로 내보이게 되고, 에러가 나지 않아 매장에서 잘못된 쿠폰이 사용된다.** 쿠폰이 1개뿐인 참여자로는 이 버그가 드러나지 않으므로 반드시 2개로 확인한다.

Run:
1. `console.log`로 `get_my_coupons`의 원본 `data`를 찍어 **실제 반환 컬럼명**을 확인한다. `coupon_id`/`coupon_type`/`is_used`/`expired_at`과 다르면 `IssuedCouponRow`와 `toIssuedCoupon()`을 실제 이름에 맞춘다. 발급 시각 컬럼 이름도 여기서 확인해 위 정렬 코드의 `created_at`을 맞춘다.
2. 한 번 뽑고 `couponId`를 기록한다.
3. 대시보드에서 `participants.roulette_joined`를 비우거나 쿨타임이 지나길 기다린 뒤 다시 뽑는다.

Expected: 2회차의 `couponId`가 1회차와 **다르고**, `/main/coupons` 발급 내역의 더 최신 행과 일치한다. 같으면 정렬이 반대이므로 정렬 코드를 고치고, 확인 결과를 설계 문서 미해결 항목 1번에 기록한다.
...
**이 태스크 없이는 가챠 확률 구간이 동작하지 않는다.** `gookbapgame`은 지금 `game_score_logs`에 아무것도 쓰지 않는다(2026-07-30 참여자 식별 스펙에서 "후속 스펙으로 분리"된 뒤 아직 구현되지 않음). draw API는 집계 시간 내 `game_score_logs`의 최고점으로 `gatcha_cases` 구간을 고르므로, 기록이 없으면 `bestScore`가 항상 0이 되어 **모든 플레이어가 최저 구간 확률 풀로만 뽑는다.**

`game_score_logs`는 Anon INSERT가 허용되어 있다(`gookbapanalyze/AGENTS.md`). 조회는 RPC가 필요하지만 여기서는 INSERT만 한다.

**Files:**
- Modify: `app/actions.ts` (파일 끝에 추가)
- Modify: `app/hooks/useGameProgress.ts` (`finishGame` 내부)

**Interfaces:**
- Consumes: 기존 `resolveParticipantId()`, 기존 `supabase`
- Produces: `export async function submitGameScore(gookbapScore: number): Promise<void>` — best-effort. 실패해도 예외를 던지지 않고 로그만 남긴다(점수 기록 실패로 결과 화면이 막히면 안 된다).

- [ ] **Step 1: 실제 컬럼명을 확인한다**

`game_score_logs`의 컬럼 구성은 코드로 완전히 확인되지 않는다. 확인된 것은 `/api/gatcha/draw`가 쓰는 `participant_id`, `gookbap_score`, `joined_time` 셋뿐이고, `ranking_view`는 `best_score`와 `gookbap_score`를 **서로 다른 값**으로 노출한다(`gookbapanalyze/AGENTS.md`). 커밋 `6af1458`에 따르면 내부 0~100 비율과 표시용 1953 환산 점수가 분리되어 있다.

Run: 대시보드 Supabase에서 `game_score_logs` 테이블 정의를 열어 컬럼과 NOT NULL 제약을 확인한다.
Expected: 컬럼 목록 확보. `best_score`가 NOT NULL이면 어떤 값을 넣어야 하는지 구자건에게 확인하고, 그 결론을 설계 문서 미해결 항목에 추가한다.

- [ ] **Step 2: 서버 액션을 추가한다**

`app/actions.ts` 끝에 추가한다. Step 1에서 확인한 컬럼명에 맞춘다.
...
- [ ] **Step 3: 게임 종료 시 호출하도록 배선한다**

`app/hooks/useGameProgress.ts`에서 `submitGameScore`를 import하고, `finishGame` 안에서 `setPhase("gameResult")` 직전에 호출한다. 최종 점수는 `calcFinalScore`가 만든 `ScoreBreakdown`의 `total`이다.
...
주의: `finishGame` 내부에서 `ScoreBreakdown`을 담고 있는 지역 변수명이 `breakdown`이 아닐 수 있다. 실제 변수명에 맞춰 쓴다.

- [ ] **Step 4: 기록이 실제로 남는지 확인한다**

Run: 로컬에서 게임을 완주한 뒤 대시보드 `/main` KPI 또는 Supabase에서 `game_score_logs`를 조회한다.
Expected: 방금 플레이한 `participant_id`로 행이 1개 생기고 `gookbap_score`가 결과 화면의 최종점수와 같다. 게임 완주율 KPI에도 잡힌다(`AGENTS.md` 3단계).

- [ ] **Step 5: 커밋한다**

```bash
git add app/actions.ts app/hooks/useGameProgress.ts
git commit -m "게임 완주 시 점수를 game_score_logs에 기록"
```

---

### Task 5C: 미완료 뽑기 표시 저장소

접속 실패로 뽑기를 못 한 경우, 다음 방문 시작 화면에 "쿠폰 뽑으러 가기" 버튼을 띄우기 위한 플래그. 자격 여부를 조회하는 API가 없으므로(draw를 실제로 호출해야만 알 수 있다) 클라이언트에 표시만 남긴다.

`localStorage`를 쓴다 — 이미 로케일 저장(`gukbap_locale`)에 쓰고 있고, 이 값은 UI 힌트일 뿐 권한이나 발급 자격과 무관해서 조작되어도 손해가 없다(눌러봐야 draw API가 다시 판정한다).

**Files:**
- Create: `app/lib/pendingDraw.ts`
- Create: `app/lib/pendingDraw.test.ts`
- Modify: `package.json:9`
...
`package.json`의 `test` 스크립트에 ` app/lib/pendingDraw.test.ts`를 추가한 뒤:

Run: `npm test`
Expected: FAIL — `Cannot find module './pendingDraw.ts'`

- [ ] **Step 3: 최소 구현을 작성한다**

`app/lib/pendingDraw.ts`:
...
```bash
git add app/lib/pendingDraw.ts app/lib/pendingDraw.test.ts package.json
git commit -m "미완료 뽑기 표시 저장소 추가"
```
...
- Produces:
  ...
  - `submitAnswers(answers: SurveyAnswerMap): Promise<boolean>` — 이미 제출한 세션이면 서버를 호출하지 않고 곧바로 `true`를 반환한다(`hasSubmittedRef`). `reset()`으로 풀리지 않는다.
  - `reset()`은 `drawStartedRef`만 되돌린다. 제출 이력은 유지된다.
...
- Produces: `export default function WheelScreen({ drawResult, isDrawing, onSpin, onNext }: { drawResult: DrawCouponResult | null; isDrawing: boolean; onSpin: () => void; onNext: () => void })`
  - `onSpin`은 마운트 시 1회 호출된다. 중복 방지는 `useCouponFlow`의 ref 가드가 담당하므로 여기서는 신경 쓰지 않는다.
  - **전용 재시도 버튼을 두지 않는다.** 하루 한 번뿐인 기능이고, 오늘의 결과의 '설문하고 쿠폰 받기'로 재진입하면 `reset()` 후 draw가 다시 실행되므로 복구 경로가 이미 있다.
...
- Modify: `app/components/StartScreen.tsx` (뽑기 진입 버튼 추가)
...
- Produces:
  ...
  - `DailyResultScreen`에 `onSurveyAgain?: () => void`, `onOpenMyCoupons?: () => void` prop 추가. 넘기지 않으면 버튼이 렌더되지 않는다.
  - `StartScreen`에 `onGoToDraw?: () => void` prop 추가. `hasPendingDraw()`가 true일 때만 넘긴다.
...
- [ ] **Step 2B: StartScreen에 뽑기 진입 버튼을 추가한다**

`app/components/StartScreen.tsx`의 props 타입에 `onGoToDraw?: () => void`를 추가하고, 기존 시작 버튼 아래에 넣는다. **prop이 없으면 렌더되지 않는다** — 접속 실패로 뽑기를 못 한 사람에게만 보여야 하기 때문이다.
...
시작 버튼(게임 시작)이 이 화면의 주행동이므로, 뽑기 진입은 설문 거절 링크와 같은 보조 스타일로 둔다.
...
- [ ] **Step 5: 접속 실패 후 복구 경로가 동작하는지 확인한다**

전용 재시도 버튼이 없다. 요청이 서버에 닿지 못했을 때는 안내 문구만 띄우고 흐름을 그대로 진행시키며, 기회는 살아 있으므로 다음 방문 시작 화면의 뽑기 버튼으로 되돌아온다.

Run: `npx tsc --noEmit && npm run build`
Expected: 빌드 통과.

그 다음:
1. `GATCHA_DRAW_API_URL`을 잘못된 주소로 두고 게임을 완주해 룰렛까지 간다.
   Expected: "지금은 접속이 원활하지 않습니다..." 문구가 뜨고, "다음"으로 오늘의 결과까지 정상 진행된다. 발급도 쿨타임 갱신도 일어나지 않는다.
2. 환경변수를 올바르게 고치고 서버를 재시작한 뒤, "처음으로"를 눌러 시작 화면으로 간다.
   Expected: **"쿠폰 뽑으러 가기" 버튼이 보인다**(`hasPendingDraw()`가 true).
3. 그 버튼을 누른다.
   Expected: 설문 안내 → 문항 화면이 뜨고, 제출하면 `hasSubmittedRef` 때문에 서버 호출 없이 즉시 룰렛으로 넘어가 쿠폰이 발급된다(응답이 두 번 쌓이지 않는다).
4. 다시 시작 화면으로 간다.
   Expected: 뽑기 버튼이 **사라져 있다**(발급 성공 시 `clearPendingDraw()`).
...
```bash
git add app/page.tsx app/hooks/useGameProgress.ts app/components/DailyResultScreen.tsx app/components/StartScreen.tsx
