...
- **RPC 응답을 화면 코드가 직접 읽지 않는다.** `gameMasterData.ts`의 `toGameMasterData`
  **한 곳**에서 도메인 타입으로 옮긴다. 저쪽 형태가 바뀌면 고칠 자리가 거기 하나다.
  - **`questions_count`가 응답에 없으면 `console.error`를 남긴다.** 없으면
    `resolveQuestionsCount`가 `STAGE_CONFIG` 폴백으로 조용히 떨어져, 대시보드에서 3개로
    맞춰도 7단계가 7문항으로 나오던 2026-08-07 이전 버그가 **에러 없이** 되살아난다.
    이 로그를 관대한 기본값으로 바꾸지 말 것.
  - 파츠는 `categories[].parts[]`에 중첩돼 온다. **파트 행에 `category_id`가 없을 수
    있으므로 부모 카테고리의 `id`를 쓴다** — 저쪽 `route.ts`가 파츠를 평탄화해놓고
    카테고리를 역추적하는 것이 그 방증이다(있었다면 그럴 이유가 없다). 평탄화한 배열에
    `p.category_id === slot.category_id` 필터를 얹으면 `undefined === n`이라 유효 슬롯이
    0개가 되고, **전 레벨이 "N단계 게임 데이터를 불러올 수 없습니다"로 죽는다.**
- **로컬 픽스처가 필요하다.** `docs/test-db/schema.sql`에 이 RPC가 없어서, 없는 채로는
  로컬에서 게임이 시작되지 않는다(예전에는 개별 조회라 게임 화면까지는 갔고 합성만
  실패했다). **실물 응답 형태를 확인하기 전에 픽스처를 손으로 지어내지 말 것** —
  프로덕션에 없는 모양을 상대로 로컬이 통과하는 상태가 더 나쁘다. `ranking_view`의
  `nickname_number`가 정확히 그 사고였다.
