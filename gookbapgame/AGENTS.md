...
# 출제는 클라이언트가 한다 (`app/actions.ts`의 `fetchGameData`)
...
- **레벨 간 중복은 구조적으로 불가능하다.** `fetchGameData`가 `.eq("level", level)`로
...
- **연속된 판 사이의 같은 배경만 실제 구멍이었다.** `preloadAllStages`가 7레벨을
  **병렬로** 뽑고 서버 액션은 직전 판을 기억하지 못하므로, 다시하기에서 같은 배경이
  ...
  - **제외가 아니라 후순위다.** 목록에서 빼면 배경이 1장뿐인 레벨에서 후보가 0개가 되어
    `fetchGameData`가 `null` → `preload.levelSessionError` → **게임이 아예 시작되지
