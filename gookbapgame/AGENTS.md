...
- **font-size는 CSS가 아니라 `useFitText`가 실측해서 정한다.** 예전엔
  `clamp(2.5rem, 19cqw, 5rem)`이었는데, 그 `19`는 "Galmuri11 라틴 대문자는 약 0.5em"이라는
  **틀린 가정**에서 역산한 값이었다. 실제로는 글자마다 0.333~0.833em이고 GAME OVER는
  가정(4.5em)보다 31% 넓은 **5.917em**이라, 모든 기기에서 창 밖으로 41~55px 넘쳤다
  (2026-08-12, iOS 실기 제보로 드러났지만 **폰트 메트릭 문제라 플랫폼 무관**이고
  데스크톱에서 창을 좁혀도 재현된다).
  - **계수를 실측값으로 다시 잡는 방식으로 되돌리지 말 것.** 라벨이 바뀔 때마다 사람이
    다시 계산해야 한다. CSS는 컨테이너 폭만 알 뿐 그 안의 글자 폭을 모르므로 원리적으로
    자동화가 안 된다 — 브라우저에게 실제 렌더 폭을 묻는 쪽이 맞다.
  - 보정식에서 **비례항과 고정항을 섞지 말 것**(`useFitText` 주석 참고). 그림자 4px까지
    폰트 크기에 비례한다고 보면 필요 이상으로 줄여놓고도 정작 넘친다. 그림자 몫은
    가운데 정렬 때문에 `4 * 2`로 잡는다 — 4px만 빼면 오른쪽에 2px밖에 안 돌아간다.
    셋 다 실측으로 확인했다.
- **`.game-cue-window`의 `height: 60px`은 창이 흔들리지 않게 하는 몫이다.** 라벨마다
  font-size가 달라지면서 `line-height: 1`인 글자 높이가 따라 변해 팝업 창이 위아래로
  움직였다(2026-08-12, 이란토). 폭을 `width: 100%`로 고정한 것과 같은 이유다.
  이 값은 `GAME_CUE_MAX_PX`와 **같아야 한다** — 한쪽만 바꾸면 조용히 어긋난다.
- **`GAME_CUE_MAX_PX`(60px)는 상한이다.** 짧은 라벨(3·START·CLEAR!)은 여기 걸려 멈추고
  긴 라벨만 줄어든다. 상한이 없으면 카운트다운의 `3` 한 글자가 창 폭을 꽉 채워 거대해진다.
  값의 근거는 2026-08-11에 튜닝된 카운트다운의 당시 실제 크기(390px 화면에서 약 60px)다 —
  **낮추면 카운트다운 연출도 같이 작아진다.** 종료 화면의 넘침은 이 값이 아니라
  `useFitText`가 막으므로, 넘친다고 이 값을 건드리는 것은 잘못된 처방이다.
- **라벨은 i18n을 타지 않는다**(2026-08-12). `GAME OVER` / `CLEAR!` / `START` 모두
  ...
  - `useFitText` 덕에 긴 라벨이 와도 넘치지는 않지만, 그건 넘침 방지일 뿐 번역 경로를
    되살릴 근거는 아니다.
- 이 건은 **로컬에서 검증된다** — 폰트 메트릭과 컨테이너 폭 문제라 실기가 필요 없다.
  320~768px에서 라벨 4종의 좌우 넘침이 0인 것과 창 높이가 고정인 것을 실측으로 확인했다.
...
- **음량 0.5는 코드가 아니라 에셋에 있다.** `docs/build-sfx.sh`가 `-af volume=0.5`로
  -6dB를 구워 넣는다. `audio.volume = 0.5`는 iOS에서 조용히 무시되므로 쓰지 말 것.
  매장에서 여러 대가 동시에 울리는 상황이라 BGM이 효과음과 같은 크기면 시끄럽다
  (2026-08-12, 이란토). 비율을 바꾸려면 스크립트를 다시 돌린다.
