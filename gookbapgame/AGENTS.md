...
- **GAME OVER는 넓은 화면에서도 53px에 머문다** — 9자라 폭 제약이 먼저 걸린다. 창을 92%
  채우므로 계수 문제가 아니라 **한 줄로는 물리적 최대치**다. 더 키우려면 `GAME`/`OVER`
  2줄로 쪼개고 창 높이를 84 → 168px로 올려야 한다(2026-08-13 실측: 2줄이면 84px 가능).
