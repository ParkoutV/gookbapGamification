 ...
 * `fetchGameData` 호출은 서로도 직전 판도 알지 못한다. 그래서 같은 레벨이 연속으로
 * 같은 배경을 고를 수 있다 — 레벨당 배경이 3장이면 확률 1/3이다. 그걸 막을 코드가
 * 지금까지 아무 데도 없었다(2026-08-15, 이란토).
 *
 * **레벨 간 중복은 애초에 불가능하다.** `fetchGameData`가 `.eq("level", level)`로
 ...
 * (로컬 픽스처가 그렇다), 그러면 후보가 0개가 되어 `fetchGameData`가 `null`을 돌려주고
