...
/**
 * 온라인몰 쿠폰의 코너 마크. 이름이 표에 안 걸릴 때만 쓰는 기본값이다 —
 * 실제 값("1원 할인 쿠폰 …")은 "할인"·"쿠폰"에 걸려 같은 🎫가 나온다.
 *
 * **`DEFAULT_COUPON_EMOJI`(음식)를 쓰지 않는 이유**: 온라인몰 쿠폰은 매장에서
 * 먹는 물건이 아니라 몰에 코드를 붙여넣는 티켓이라, 음식 그림이 붙으면 무엇을
 * 받았는지 오해하게 된다.
 *
 * **VS16(U+FE0F)을 붙이지 말 것** — 위 주석과 같은 이유다. U+1F3AB는 서브셋
 * 폰트(`docs/build-emoji-font.sh`의 UNICODES)에 이미 들어 있으므로 폰트를
 * 다시 만들 필요가 없다. 다른 이모지로 바꾼다면 그 목록부터 확인할 것.
 */
export const ONLINE_COUPON_EMOJI = "\u{1F3AB}";
