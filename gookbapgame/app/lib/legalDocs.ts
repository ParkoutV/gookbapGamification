 ...
 *
 * **2026-08-20부터 이것은 폴백이다.** 운영 본문은 대시보드가 관리하는 `agreements`
 * 테이블에서 오고(`app/lib/agreements.ts`, `fetchAgreements`), 조회에 실패하거나 요청
 * 로케일 본문이 DB에 없을 때 여기 것이 뜬다 — 개인정보처리방침은 최초 실행 고지의
 * 대상이라 "잠깐 비어 있음"이 곧 고지 실패이기 때문이다. **지우지 말 것.**
 * 회사가 본문을 고치면 DB와 이 파일을 함께 갱신해야 완전히 반영된다.
 *
 ...
 *
 * **시행일은 2026년 8월 21일이다**(2026-08-19, 이란토 지정). 그전까지
 * `[YYYY년 MM월 DD일]` 자리표시자로 비워 두고 기획자 확인을 기다렸는데 —
 * 본사 방침의 시행일(2025-09-01)은 이 문서의 것이 아니고, 서브도메인 연결일
 * (2026-08-17)과 방침 확정일 중 무엇을 쓸지 정해지지 않았기 때문이다 —
 * 이란토가 그 날짜로 정했다. 기획자가 다른 날짜를 주면 그쪽이 우선한다.
 *
 * `legalDocs.test.ts`가 (1) 본문에 자리표시자가 남아 있지 않고 (2) 연락처·시행일
 * 줄이 ko·en 양쪽에 실제 값으로 살아 있는지를 검사한다. 전사 중에 문서 끝이 잘려 그
