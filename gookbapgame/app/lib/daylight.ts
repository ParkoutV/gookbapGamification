 ...
 * 흰 글자로 뒤집고 이 그라데이션을 깔면 푸터 자리 국소 최악이 **9.44:1**이다
 * (2026-08-17 배경 교체분 실측. 그 전 4K 6장에서는 7.22:1이었다 — 새 그림은 하단이
 * 전망대 난간이라 어두워 대비가 올랐다).
 *
 * ## 칩(사각형 배경)을 쓰지 않는 이유
 *
 ...
 * 항상 이긴다"는 전제가 깨진다 — 지금은 `day`조차 하단이 전망대 난간이라 어둡지만,
 * 하단이 밝은 그림(하늘·바다가 화면 아래까지 오는 구도)이 들어오면 이 전제가 무너진다.
 *
 * 재는 법(2026-08-17에 복원한 절차):
 *  1. 원본을 390×844(iPhone 16 Pro CSS px)에 `object-fit: cover` + center로 맞춘다.
 *  2. 하단 `FOOTER_SCRIM_HEIGHT`px에 이 그라데이션을 합성한다.
 *  3. **푸터 글자가 놓이는 띠만** 본다 — 화면 하단에서 `p-6`(24px) 위로 49px
 *     (개인정보처리방침 버튼 + gap + 저작권 줄). 스크림 밴드 전체로 재면 위쪽이
 *     사진 그대로라 1.01:1이 나온다 — 그 수치는 이 값과 비교 대상이 아니다.
 *  4. 그 띠에서 **가장 밝은 픽셀**의 WCAG 상대휘도로 흰색과의 대비를 낸다.
