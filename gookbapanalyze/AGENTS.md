...
직접적인 테이블이 아니며, `participants` 조회 차단 정책을 보완하여 유저 랭킹보드 표시 목적으로 가공된 가상의 뷰(View)입니다. 보안을 위해 `participant_id`는 의도적으로 제외되었습니다.
...
6. **전체 랭킹 조회 (`ranking_view`)**
   ...
   - `ranking_view`는 보안상 민감한 데이터(`participant_id` 등)를 철저히 제외하고, 최고 점수 기록인 `nickname_first`, `nickname_last`(다국어 JSONB), `nickname_number`, `best_score`, `gookbap_score`, `joined_time`을 제공합니다. (사전 필터링 없이 1명의 유저가 낸 **모든 기록이 중복을 포함하여 반환**되므로 프론트엔드 구현단에서 주간/월간 랭킹 등 용도에 맞게 필터링해야 합니다.)
...
9. **자동 번역 버튼 (TranslationButton) 컴포넌트 사용 가이드:** 
   - **아키텍처**: 브라우저 부하를 없애기 위해 서버사이드 API (`/api/translate`)를 거쳐 구글 번역 서비스(GAS 우회 방식)를 호출합니다. 
   - **제한 및 로깅**: 구글 번역 API는 일일 5,000회(PST 자정 리셋)의 넉넉한 한도를 가집니다. `daily_translation_usage` DB 테이블과 `increment_translation_usage` RPC를 통해 백엔드에서 실시간으로 사용량을 추적하며, UI상에 잔여 횟수 알림을 표시해 줍니다.
   - **레이아웃 배치 원칙**:
     - **단일/개별 필드 번역**: 다국어 입력 폼(예: 제목 입력칸 등)의 **라벨 우측 끝 또는 인풋 박스 상단** 영역에 `compact={true}` 속성을 주어 작은 아이콘 형태로 튀지 않게 배치합니다.
     - **일괄 번역 (Form 전체)**: 여러 다국어 필드를 한 번에 채워야 할 경우, 페이지나 Card 컨테이너의 **우측 상단 액션 바(Action bar)** 영역에 `compact={false}` 기본 버튼으로 배치하여 눈에 띄게 유도합니다.
   - **리소스 절약 (existingTranslations prop)**: 번역 버튼 호출 시, 이미 번역값이 채워진 칸은 API를 낭비하지 않도록 사전에 차단해야 합니다. 반드시 부모 컴포넌트에서 `existingTranslations` prop으로 현재 다국어 입력 상태를 전달하여, 비어있는 항목만 정확히 번역되도록 최적화해야 합니다.
